...
}else{ if($_POST['Name']!=$user->getName()){
    $user->setName($_POST['Name']);
    $name=$_POST['Name'];
    $sql = "UPDATE batch_01 SET Name= '$name' WHERE id = '$id'";
    mysqli_query($conn,$sql);

} if($_POST['Address']!=$user->getAddress()){
    $user->setAddress($_POST['Address']);
    $Address=$_POST['Address'];
    $sql = "UPDATE batch_01 SET Address= '$Address' WHERE id = '$id'";
    mysqli_query($conn,$sql);

} if($_POST['NIC']!=$user->getNic()){
    $user->setNic($_POST['NIC']);
    $NIC=$_POST['NIC'];
    $sql = "UPDATE batch_01 SET NIC= '$NIC' WHERE id = '$id'";
    mysqli_query($conn,$sql);

}if($_POST['DateOfBirth']!=$user->getDateOfBirth()){
    $user->setDateOfBirth($_POST['DateOfBirth']);
    $dateOfBirth=$_POST['DateOfBirth'];
    $sql = "UPDATE batch_01 SET DateOfBirth= '$dateOfBirth' WHERE id = '$id'";
    mysqli_query($conn,$sql);

}if($_POST['ContactNo']!=$user->getContactNo()){
    $user->setContactNo($_POST['ContactNo']);
    $ContactNo=$_POST['ContactNo'];
    $sql = "UPDATE batch_01 SET Contact = '$ContactNo' WHERE id = '$id'";
    mysqli_query($conn,$sql);

} if($_POST['Email']!=$user->getEmail()){
    $user->setEmail($_POST['Email']);
    $Email=$_POST['Email'];
    $sql = "UPDATE batch_01 SET Email= '$Email' WHERE id = '$id'";
    mysqli_query($conn,$sql);

}if($_POST['Password1']!=$user->getPassword()) {
    $user->setPassword($_POST['Password1']);
    $Password = $_POST['Password1'];
    $sql = "UPDATE batch_01 SET Password= '$Password' WHERE id = '$id'";
    mysqli_query($conn, $sql);
